...
class SimulationThread(QThread):
    ...
    def update_simulation_step(self):
        """Update one simulation step - optimized for performance"""
        ...
        while batch_start < len(self.agents):
            ...
            self._update_agent_batch(self.agents[batch_start:batch_end], accelerated_dt)
        ...
        if self.step % self.update_frequency == 0:
            with QMutexLocker(self.update_mutex):
                # Create lightweight agent data for visualization
                agent_data = self._prepare_agent_data()
                # Pass current simulation time for interpolation
                for agent in agent_data:
                    agent.simulation_time = self.simulation_time
                self.agents_updated.emit(agent_data)
                self.last_visual_update = self.step
    ...
    def _update_agent_batch(self, agent_batch, accelerated_dt):
        """Update a batch of agents efficiently"""
